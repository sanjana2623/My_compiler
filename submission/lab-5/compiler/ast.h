...
public:
	...
	virtual int insertRtl(vector<Rtl_Node*> &Rtl_list, int regindex){}
};
...
public:
	...
	int insertRtl(vector<Rtl_Node*> &Rtl_list, int regindex){}
...
public:
	...
	Float_Ast(DataType dtype, NodeText label, double data);
	void printAst(ostream& aout);
	Leaf_Tac* insertTac(vector<Tac_Node*> &Tac_list);
};
...
public:
	...
	vector<Rtl_Node*> Rtl_list;

	// #####################for AST##############################

	void makeAstFile(ostream& aout){
...
Formal Parameters:";
	...
	// #####################for TAC##############################
	//  common function to generate TAC list for TAC and RTL
	void makeTac(){
		...
		else{} // tac already created when -tac or -rtl is used so no repetation
	}
	void makeTacFile(ostream& tout){
	...
	// #####################for RTL##############################
	void makeRtlFile(ostream& rout){

		// first create TAC list from AST list
		this->makeTac();
		// then create RTL list from TAC list

		// now initialise temp to reg vector
		temp_toReg = vector<int>(temp_index, -1);
		//  now initialise spim Registers
		Reg_alloc_obj->makeReg();
		
		for(auto x:this->Tac_list){
			x->insertRtl(this->Rtl_list, -1);
		}

		if(! (this->Rtl_list).empty() ){ 
			rout<<"**PROCEDURE: main\n**BEGIN: RTL Statements\n";
		}
		for(auto x:this->Rtl_list){
			x->printRtl(rout);
		}

		if(! (this->Rtl_list).empty() ){ 
			rout<<"**END: RTL Statements\n";
		}
	}
};
/////////////////////////////////////////////////