...
Float_Ast::Float_Ast(DataType dtype, NodeText label, float data){
	this->dtype = dtype;
	this->label = label;
	this->data = data;
}
...
Asgn_Ast::Asgn_Ast(DataType dtype, NodeText label, Name_Ast* LHS, Ast_Node* RHS){
...
Return_Ast::Return_Ast(NodeText label, Exp_Ast* data){
	this->dtype = data->dtype;
	this->label = label;
	this->data = data;
}

void Return_Ast::printAst(ostream& aout){
	aout<<"\nReturn:";
	this->data->printAst(aout);
}

Fun_call_Ast::Fun_call_Ast(NodeText label,Fun_def* fun, ArgList_class* arglist){
	if((fun->param_L->dtype).size() != (arglist->arg_list).size()){
		semantic_error();
	}
	for(int i=0; i< (arglist->arg_list).size(); i++){
		if ( ((arglist->arg_list)[i])->dtype != (fun->param_L->dtype)[i]){
			semantic_error();
		}
	}
	this->dtype = fun->rtype;
	this->label = label;
	this->fun = fun;
	this->arglist = arglist;
}

void Fun_call_Ast::printAst(ostream& aout){
	aout<<"\nFN CALL: ";
	aout<<this->fun->fname<<"_(";
	this->arglist->printAst(aout);
	aout<<")";

}

void ArgList_class::printAst(ostream& aout){ 
...
void Fun_def::printAst(ostream& aout){
	aout<<"\n**PROCEDURE: "<<this->fname;
	if(this->fname != "main"){ aout<<"_"; }
	
	aout<<"\nReturn Type: ";
	printEnumDT(aout, this->rtype);

	aout<<"\nFormal Parameters:";
	this->param_L->printAst(aout);
